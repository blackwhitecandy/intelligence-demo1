...
;(function($,window,document,undefined){
	...
	var defaults = {
		...
		count:3,				//当前页前后分页个数
		jump:false,				//跳转到指定页数
		jumpIptCls:'num',	//文本框内容
		jumpBtnCls:'jump',	//跳转按钮
		jumpBtn:'确定',			//跳转按钮文本
	...
	var Pagination = function(element,options){
		...
		this.filling = function(index){
			...
			if(current >= opts.count * 2 && current != 1 && pageCount != opts.count){
				...
				html += opts.coping ? '<a href="javascript:;" data-page="1">'+home+'</a><span class="dot">...</span>' : '';
			}
			...
			if(current + opts.count < pageCount && current >= 1 && pageCount > opts.count){
				...
				html += opts.coping ? '<span class="dot">...</span><a href="javascript:;" data-page="'+pageCount+'">'+end+'</a>' : '';
			}
			...
			html += opts.jump ? '<p class="skip">共<b>'+end+'</b>页 到第<input type="text" class="'+opts.jumpIptCls+'">页<a href="javascript:;" class="'+opts.jumpBtnCls+'">'+opts.jumpBtn+'</a></p>' : '';
